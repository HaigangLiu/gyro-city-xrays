...
class CamGenerator():
    ...
    def process_one_image(self, image_tensor, which_class):
        # try:# need a specialized dataloader
        #     self.image_tensor, self.text_label, self.list_of_boxes = next(iter(self.specialized_dataloader))
        # except:
        #     raise ValueError('Need to pass a PyTorch DataLoader object as the positional argument')

        # assert len(self.text_label) == 1, 'Set the batch size of dataloader to 1'
    ...
    def _post_processing_for_cam(gradient_CAM):
        ...
        gradient_CAM = cv2.resize(gradient_CAM.numpy(), (1024, 1024))
        gradient_CAM = cv2.applyColorMap(np.uint8(gradient_CAM * 255.0), cv2.COLORMAP_JET)

        return gradient_CAM

    def show(self):
        self.image_numpy = cv2.resize(self.image_numpy, (1024, 1024))
        ...
        # true_name = 'Positive' if self.text_label == 1 else 'Negative'
        fig = plt.figure()
    ...
    def batch_testing_engine(self):
        ...
        gradient_history = []
        gradient_cam_history = []

        for self.image_name_, self.image_tensor, self.text_label, self.bbox in self.specialized_dataloader:
            self.image_name_ = self.image_name_[0]
            self.process_one_image(self.image_tensor, self.text_label[0])
            self.find_gradient_CAM()
            feature_maps_history.append(self.feature_maps)
            gradient_history.append(self.gradient)
            gradient_cam_history.append(self.gradient_CAM)
...
if __name__ == '__main__':
    ...
    from data_utilities import SpecializedContructorForCAM
    ...
    dataloader_cam = DataLoader(dataset = dataset_cam,batch_size = 1, shuffle = False, num_workers = 4, pin_memory = True)

    cam_generator = CamGenerator(model,'features.denseblock4.denselayer16.conv2', dataloader_cam)
    cam_generator.batch_testing_engine()
