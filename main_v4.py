...
sampler_train = Sampler(train_dataset, 'both', sample_size=75000).generate_sampler()
...
model_ft = ModelGenerator(MODEL_NAME,
                    pretrain = True,
                    freeze_all_layers_before_last = False).\
...
if BINARY:
    cross_entropy_loss = customize_cross_entropy_function(train_dataset, 'auto', verbose = False)
else:
