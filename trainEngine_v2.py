...
class ModelTrainingAndTesting:
    def __init__(self, model_identifier, model_cache_dir,  number_of_classes, learning_rate = 0.001,  model_ft = None, criterion = None, optimizer = 'Adam'):
        ...
        '''
        NOTE:
        1. To do weigthed cross entropy, user needs to pass in a customized loss function.
        2. This class can be used for test purpose by passing the trained model and skip the training session. An example would be:
    ...
    def report_on_testset_ten_crop(self, test_dataloader):
        ...
        if self.number_of_classes > 2:
            ...
            lp = pred_prob.cpu().detach().numpy()
            report_stats = stats_calculator_binary([lp.ravel(), gt.ravel()])
