...
class CamGenerator():
    ...
    def show(self):
        try:
            guessed_name = 'Positive' if self.argmax == 1 else 'Negative'
...
if __name__ == '__main__':
    ...
    model = torch.load('/Users/haigangliu/Desktop/transfer_image_net.pth.tar', map_location='cpu')
    ...
    def make_binary_prediction(model_loaded, one_image_loader):
        for images in one_image_loader:
            images = images.to(device)
            with torch.set_grad_enabled(mode = False):
                outputs = model_loaded(images)
                _, pred = torch.max(outputs, 1)
        return [pred.item(), outputs[0][1].item()]

    class SingleImageDataSetMaker(Dataset):

        def __init__(self, root_dir, transform = None):
            self.root_dir = root_dir
            self.image_files = [file for file in os.listdir(self.root_dir) if file.endswith('.jpeg')]
    ...
    single_image_dataset = SingleImageDataSetMaker('/Users/haigangliu/Desktop/test/', transforms.Compose([transforms.Resize([224,224]),transforms.ToTensor(),NORMALIZE]))
    one_image_loader = DataLoader(dataset = single_image_dataset, batch_size = 1, num_workers = 1, pin_memory = True)
    #pred_and_prob = make_binary_prediction(loaded_model, one_image_loader)


    cam_generator = CamGenerator(model, 'features.denseblock4.denselayer16.conv2')
    cam_generator.load_data(one_image_loader)
